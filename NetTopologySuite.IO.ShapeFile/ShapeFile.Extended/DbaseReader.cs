    ...
    public class DbaseReader : IEnumerable<IAttributesTable>, IDisposable
    {
        ...
        public IAttributesTable ReadEntry(int index)
        {
            ...
            for (int i = 0; i < data.Count; i++)
            {
                ...
                byte[] utfBytes = utf8.GetBytes(data[i].ToString());
                byte[] isoBytes = Encoding.Convert(utf8, iso, utfBytes);
                tbl.Add(m_Header.Fields[i].Name, iso.GetString(isoBytes).Trim());
            }
                ...
                switch (tempFieldType)
                {
                    ...
                    case 'C':   // character record.
                        ...
                        else
                        {
                            ...
                            byte[] isoBytes = Encoding.Convert(utf8, iso, buf);
                            tempObject = iso.GetString(isoBytes);
                        }
                        break;
